 ...
 * @LastEditTime: 2023-10-18 16:48:06
 */
/*
...
////////////////////////////////////////////////////////////////////////////////
// Default constructor
int AMCLLaser::grid_weight_ = 0;
int AMCLLaser::landmark_weight_ = 0;
...
void AMCLLaser::SetModelLikelihoodField(double z_hit, double z_rand,
  ...
  // SLAM_WARN("进入SetModelLikelihoodField!!!!!!!!!!!!!\n");
  this->model_type = LASER_MODEL_LIKELIHOOD_FIELD;
  this->z_hit = z_hit;
  this->z_rand = z_rand;
  this->sigma_hit = sigma_hit;
  // 是否使用权重区域改变似然域地图
  SLAM_INFO("use_weighted_areas: %d", use_weighted_areas_);
  if (use_weighted_areas_) {
    SLAM_INFO("likelihood map_data_file_path: %s/ale.smap",
              map_data_file_path_.c_str());
    weighted_areas = new WeightedAreas(map_data_file_path_ + "/ale.smap");
  }
  map_update_cspace(this->map, max_occ_dist, max_occ_dist * 2, weighted_areas);
  // SLAM_WARN("退出SetModelLikelihoodField!!!!!!!!!!!!!\n");
}
...
bool AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data) {
  ...
  if (this->model_type == LASER_MODEL_BEAM)
    ...
    // SLAM_WARN("选择LikelihoodFieldModel模型\n");
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data,
  ...
  for (j = 0; j < set->sample_count; j++) {
    ...
    // SLAM_WARN("反光柱激光点数目%d\n", data->range_count2);
    // SLAM_WARN("是否使用反光柱更新粒子权重  %d\n", data->use_landmark);
    if (data->use_landmark && data->ranges2 != nullptr &&
        data->range_count2 != 0) {
      if (landmark_weight_ != 0) {
        for (i = 0; i < data->range_count2; i++) {
          pz = 0.0;
          // SLAM_WARN("添加反光柱粒子权重\n");
          // Compute the endpoint of the beam
          hit.v[0] = pose.v[0] + data->ranges2[i][0] * cos(pose.v[2]) -
                     data->ranges2[i][1] * sin(pose.v[2]);
          hit.v[1] = pose.v[1] + data->ranges2[i][0] * sin(pose.v[2]) +
                     data->ranges2[i][1] * cos(pose.v[2]);
          // Convert to map grid coords.
          int mi = MAP_GXWX(self->map, hit.v[0]);
          int mj = MAP_GYWY(self->map, hit.v[1]);

          // Part 1: Get distance from the hit to closest obstacle.
          // Off-map penalized as max distance
          if (!MAP_VALID(self->map, mi, mj)) {
            z = self->map->max_occ_dist_land;
          } else {
            z = self->map->cells[MAP_INDEX(self->map, mi, mj)]
                    .occ_dist_to_land_mark;
          }
          // SLAM_WARN("##############z %d", pz);
          // Gaussian model
          // NOTE: this should have a normalization of 1/(sqrt(2pi)*sigma)
          pz += self->z_hit * exp(-(z * z) / (z_hit_denom * landmark_weight_));
    ...
    if (grid_weight_ != 0) {
      for (i = 0; i < data->range_count; i += step) {
        ...
        // NOTE: this should have a normalization of 1/(sqrt(2pi)*sigma)
        pz += self->z_hit * exp(-(z * z) / (z_hit_denom * grid_weight_));
        // Part 2: random measurements
        pz += self->z_rand * z_rand_mult;
  ...
  // 保存当前帧weight到文件
  // std::ofstream out("./Likelihood_weight.xlsx", std::ios::app);
  // if(out.is_open()) {
  //   out << "当前LikelihoodFieldModel概率:\t";
  //   out << total_weight << '\n';
  // }
  return (total_weight);
}
