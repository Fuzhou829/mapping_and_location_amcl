  ...
  : m_pMapper(pMapper)
{
  ...
  m_pLoopLandmarkMatcher = LandmarkMatcher::Create(pMapper);
  assert(m_pLoopLandmarkMatcher);
...
void MapperGraph::AddEdges(LocalizedRangeScan* pScan,
  ...
  // TODO(r) link to other same qr info ladar

  if (!means.empty()) {
    pScan->SetSensorPose(ComputeWeightedMean(means, covariances));
    // FIXME(r)数据融合计算位姿及协方差矩阵
    Pose2 kf_mean_pos;
    Matrix3 kf_cov;
    ComputeMeanAndCovariance(means, covariances, &kf_mean_pos, &kf_cov);
    pScan->SetCovariance(kf_cov);
  }
}

kt_bool MapperGraph::TryCloseLoop(
  LocalizedRangeScan* pScan, const Name& rSensorName) {
  kt_bool loopClosed = false;

  kt_int32u scanIndex = 0;

  if (m_pMapper->m_pUselandmarkConstraints->GetValue() &&
      m_pMapper->m_pMapperSensorManager->GetLastScan(
      pScan->GetSensorName()) != NULL) {

    LocalizedRangeScanVector candidateChain =
      m_pLoopLandmarkMatcher->FindLoopClosureLadar(pScan);
    if (candidateChain.size() > 2) {
      ...
      LocalizedRangeScan tmpScan(pScan->GetSensorName(),
                                  ...
                                  pScan->GetIntensitiesReadingsVector());
      ...
      kt_double fineResponse =
        m_pMapper->m_pSequentialScanMatcher->MatchScan(&tmpScan, candidateChain,
                                                  bestPose, covariance, false);
      m_pMapper->FireBeginLoopClosure("Closing loop...");

      pScan->SetSensorPose(bestPose);
      SLAM_INFO("begin land mark link to scan~~~~");
      const_forEach(LocalizedRangeScanVector, &candidateChain) {
        LinkScans(*iter, pScan, bestPose, covariance);
      }
      SLAM_INFO("end land mark link to scan~~~~");
      ...
      //   Matrix3 rCovariance;
      //   rCovariance.SetToIdentity();
      //   if (m_pLoopLandmarkMatcher->IsGetLoopScan(
      //       pScan, pCandidateScan, bestPose, rCovariance)) {
      //     loopClosed = true;
      //     pScan->SetSensorPose(bestPose);
      //     LinkScans(pCandidateScan, pScan, bestPose, rCovariance);
      //     CorrectPoses();
      //     // 更新协方差矩阵
      //     pScan->SetCovariance(rCovariance);
      // }
    // }
  }
  if (m_pMapper->m_pUselandmarkConstraints->GetValue()) return loopClosed;
...
void MapperGraph::LinkScans(LocalizedRangeScan* pFromScan,
...
Pose2 MapperGraph::ComputeWeightedMean(const Pose2Vector& rMeans,
...
void MapperGraph::ComputeMeanAndCovariance(const Pose2Vector& rMeans,
  const std::vector<Matrix3>& rCovariances, Pose2* kf_mean_pos,
  Matrix3* covariance) const {
  assert(rMeans.size() == rCovariances.size());
  int size = static_cast<int>(rMeans.size());
  if (1 == size) {
    *kf_mean_pos = rMeans[0];
    *covariance = rCovariances[0];
  }
  Matrix3 cov_1 = rCovariances[0];
  Pose2 pos_1 = rMeans[0];
  for (int i = 1; i < size; i++) {
    Matrix3 square_cov1 = cov_1 * cov_1;
    Matrix3 square_cov2 = rCovariances[i] * rCovariances[i];
    Matrix3 square_cov;
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        square_cov(i, j) = square_cov1(i, j) + square_cov2(i, j);
      }
    }
    if (!square_cov.InverseFast(square_cov, 1e-14)) return;
    Matrix3 inverses = square_cov.Inverse();
    Matrix3 k = square_cov1 * inverses;
    Matrix3 identity;
    identity.SetToIdentity();
    Matrix3 temp_matrix;
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        temp_matrix(i, j) = identity(i, j) + k(i, j);
      }
    }
    *kf_mean_pos = pos_1 + k * (rMeans[i] - pos_1);
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        (*covariance)(i, j) = (temp_matrix * temp_matrix * square_cov1)(i, j) +
                              (k * k * square_cov2)(i, j);
      }
    }
    cov_1 = *covariance;
...
Mapper::~Mapper()
{
  ...
  // if (m_pUseScanMatching != nullptr)
  //   delete m_pUseScanMatching;
  // if (m_pUseScanBarycenter != nullptr)
  //   delete m_pUseScanBarycenter;
  // if (m_pMinimumTimeInterval != nullptr)
  //   delete m_pMinimumTimeInterval;
  // if (m_pMinimumTravelDistance != nullptr)
  //   delete m_pMinimumTravelDistance;
  // if (m_pMinimumTravelHeading != nullptr)
  //   delete m_pMinimumTravelHeading;
  // if (m_pScanBufferSize != nullptr)
  //   delete m_pScanBufferSize;
  // if (m_pScanBufferMaximumScanDistance != nullptr)
  //   delete m_pScanBufferMaximumScanDistance;
  // if (m_pLinkMatchMinimumResponseFine != nullptr)
  //   delete m_pLinkMatchMinimumResponseFine;
  // if (m_pLinkScanMaximumDistance != nullptr)
  //   delete m_pLinkScanMaximumDistance;
  // if (m_pLoopSearchMaximumDistance != nullptr)
  //   delete m_pLoopSearchMaximumDistance;
  // if (m_pDoLoopClosing != nullptr)
  //   delete m_pDoLoopClosing;
  // if (m_pLoopMatchMinimumChainSize != nullptr)
  //   delete m_pLoopMatchMinimumChainSize;
  // if (m_pLoopMatchMaximumVarianceCoarse != nullptr)
  //   delete m_pLoopMatchMaximumVarianceCoarse;
  // if (m_pLoopMatchMinimumResponseFine != nullptr)
  //   delete m_pLoopMatchMinimumResponseFine;
  // if (m_pCorrelationSearchSpaceDimension != nullptr)
  //   delete m_pCorrelationSearchSpaceDimension;
  // if (m_pCorrelationSearchSpaceResolution != nullptr)
  //   delete m_pCorrelationSearchSpaceResolution;
  // if (m_pCorrelationSearchSpaceSmearDeviation != nullptr)
  //   delete m_pCorrelationSearchSpaceSmearDeviation;
  // if (m_pLoopSearchSpaceDimension != nullptr)
  //   delete m_pLoopSearchSpaceDimension;
  // if (m_pLoopSearchSpaceResolution != nullptr)
  //   delete m_pLoopSearchSpaceResolution;
  // if (m_pLoopSearchSpaceSmearDeviation != nullptr)
  //   delete m_pLoopSearchSpaceSmearDeviation;
  // if (m_pDistanceVariancePenalty != nullptr)
  //   delete m_pDistanceVariancePenalty;
  // if (m_pAngleVariancePenalty != nullptr)
  //   delete m_pAngleVariancePenalty;
  // if (m_pFineSearchAngleOffset != nullptr)
  //   delete m_pFineSearchAngleOffset;
  // if (m_pCoarseSearchAngleOffset != nullptr)
  //   delete m_pCoarseSearchAngleOffset;
  // if (m_pCoarseAngleResolution != nullptr)
  //   delete m_pCoarseAngleResolution;
  // if (m_pMinimumAnglePenalty != nullptr)
  //   delete m_pMinimumAnglePenalty;
  // if (m_pMinimumDistancePenalty != nullptr)
  //   delete m_pMinimumDistancePenalty;
  // if (m_pUseResponseExpansion != nullptr)
  //   delete m_pUseResponseExpansion;
}

void Mapper::InitializeParameters()
{
  m_pUselandmarkConstraints = new Parameter<kt_bool>(
      "UselandmarkConstraints",
      "When set to true, the mapper will use a landmark constraints replace"
      "scan match. " ,
      false,
      GetParameterManager());
...
bool Mapper::getParamUselandmarkConstraints()
{
  return static_cast<bool>(m_pUselandmarkConstraints->GetValue());
}
...
// General Parameters
void Mapper::setParamUselandmarkConstraints(bool b)
{
  m_pUselandmarkConstraints->SetValue((kt_bool)b);
}
...
void Mapper::Initialize(kt_double rangeThreshold)
{
  if (m_Initialized == false)
  {
    ...
    m_pLandmarkMatcher = LandmarkMatcher::Create(this);

    assert(m_pLandmarkMatcher);
...
kt_bool Mapper::Process(LocalizedRangeScan* pScan) {
  if (pScan != NULL) {
    ...
    if (m_Initialized == false) {
      ...
      m_pLandmarkMatcher->SetLadarInRobot(
        pLaserRangeFinder->GetOffsetPose().GetX(),
        pLaserRangeFinder->GetOffsetPose().GetY(),
        pLaserRangeFinder->GetOffsetPose().GetHeading());
      m_pLandmarkMatcher->SetQrInRobot(
        pLaserRangeFinder->GetQrOffsetPose().GetX(),
        pLaserRangeFinder->GetQrOffsetPose().GetY(),
        pLaserRangeFinder->GetQrOffsetPose().GetHeading());
      m_pGraph->SetLadarInRobot(
        pLaserRangeFinder->GetOffsetPose().GetX(),
        pLaserRangeFinder->GetOffsetPose().GetY(),
        pLaserRangeFinder->GetOffsetPose().GetHeading());
      m_pGraph->SetQrInRobot(
        pLaserRangeFinder->GetQrOffsetPose().GetX(),
        pLaserRangeFinder->GetQrOffsetPose().GetY(),
        pLaserRangeFinder->GetQrOffsetPose().GetHeading());
    }
    ...
    /*
    * test if scan is outside minimum boundary or
    * heading is larger then minimum heading
    */
    ...
    // correct scan from landmark
    if (m_pUselandmarkConstraints->GetValue() && pLastScan != NULL) {
      Pose2 bestPose;
      covariance.SetToIdentity();
      if (m_pLandmarkMatcher->MatchScan(pScan,
          *m_pMapperSensorManager->GetLastScan(pScan->GetSensorName()),
          bestPose, covariance)) {
        // TODO(r) 插值！！！和协方差！！！！！
        Pose2 last_pose = pScan->GetSensorPose();
        last_pose.SetHeading(bestPose.GetHeading());
        pScan->SetSensorPose(last_pose);
        // pScan->SetCovariance(covariance);
      }
    }
