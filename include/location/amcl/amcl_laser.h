 ...
 * @Version: 1.0
 * @Date: 2022-09-28 21:23:05
 * @LastEditTime: 2023-08-07 17:22:54
 */
/*
...
class AMCLLaserData : public AMCLSensorData {
 public:
  ...
  virtual ~AMCLLaserData() {
    delete[] ranges;
    if (ranges2 != nullptr) delete ranges2;
    ranges = nullptr;
    ranges2 = nullptr;
  }
...
class AMCLLaser : public AMCLSensor {
 ...
 public:
  void SetModelBeam(double z_hit, double z_short, double z_max, double z_rand,
                    double sigma_hit, double labda_short, double chi_outlier);

 public:
  WeightedAreas *weighted_areas = NULL;
  void SetUseWeightedAreas(bool use_weighted_areas) {
    use_weighted_areas_ = use_weighted_areas;
  }

  void SetUseLandmark(bool use_landmark) { use_land_mark_ = use_landmark; }

  void SetWeightedAreasMapFilePath(std::string map_data_file_path) {
    map_data_file_path_ = map_data_file_path;
  }

 public:
  void SetModelLikelihoodField(double z_hit, double z_rand, double sigma_hit,
                               double max_occ_dist);

  // a more probabilistically correct model - also with the option to do beam
  // skipping
 public:
  void SetModelLikelihoodFieldProb(double z_hit, double z_rand,
 ...
 public:
  void SetLaserPose(const pf_vector_t &laser_pose) {
    this->laser_pose = laser_pose;
  }

 public:
  void SetGridWeight(int weight) {
    grid_weight_ = weight;
    SLAM_DEBUG("！！！！！！！！！！！！栅格权重%d\n", grid_weight_);
  }

 public:
  void SetLandmarkWeight(int weight) {
    landmark_weight_ = weight;
    SLAM_DEBUG("！！！！！！！！！！！！反光柱权重%d\n", landmark_weight_);
  }
 ...
 private:
  double beam_skip_threshold;
  // threshold for the ratio of invalid beams - at which all beams are
  // integrated to the likelihoods this would be an error condition
 private:
  double beam_skip_error_threshold;
 ...
 private:
  ...
  bool use_weighted_areas_;  // 是否使用权重区域
  bool use_land_mark_;
  static int grid_weight_;
  static int landmark_weight_;
  std::string map_data_file_path_;  // 所用地图路径
};
