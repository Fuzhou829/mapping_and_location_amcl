...
namespace karto
{
  ...
  public:
    ...
    void SetQrInRobot(float x, float y, float t) {
      m_pLoopLandmarkMatcher->SetQrInRobot(x, y, t);
    }
    /**
     * set qr offset
     */
    void SetLadarInRobot(float x, float y, float t) {
      m_pLoopLandmarkMatcher->SetLadarInRobot(x, y, t);
    }
  ...
  private:
    ...
    /**
    * @brief: 利用kf计算均值及协方差（均值暂时不采用，和上面的公式做对比）
    * @return {*}
    */
    void ComputeMeanAndCovariance(const Pose2Vector& rMeans,
  ...
  private:
    ...
    /**
     * Landmark Matcher for loop closures
     */
    LandmarkMatcher* m_pLoopLandmarkMatcher;
  ...
  public:
    ...
    inline void AddScan(LocalizedRangeScan* pScan, kt_int32s uniqueId)
    {
      ...
      if (pScan->IsGetLandmarkConstraints(&constraints_info)) {
        constraints_info.last_scan_id = GetLastScan()->GetStateId();
        constraints_info.current_scan_id = pScan->GetStateId();
        pScan->SetLandmarkConstraints(constraints_info);
      }
  ...
  public:
    /**
     * Allocate memory needed for mapping_and_location
     * @param rangeThreshold
     */
  ...
  private:
    ...
    // 是否使用
    Parameter<kt_bool>* m_pUselandmarkConstraints;
  ...
  public:
    ...
    /* Getters */
    // General Parameters
    bool getParamUselandmarkConstraints();
    ...
    double getScanMatchDistanceThreshold();
    double getScanMatchThetaThreshold();

    /* Setters */
    // General Parameters
    void setParamUselandmarkConstraints(bool b);
    ...
    void setScanMatchDistanceThreshold(double d);
    void setScanMatchThetaThreshold(double d);
  };
}  // namespace karto
}  // namespace mapping_and_location
}  // namespace data_process
