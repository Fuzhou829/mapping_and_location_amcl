...
#include "Eigen/Dense"
#include "karto_mapping/Math.h"
#include "karto_mapping/Macros.h"
#include "karto_mapping/Types.h"
...
struct LandmarkConstraintsInfo {
  LandmarkConstraintsInfo() {
    landmark_id = -1;
    is_get_landmark = false;
    time_stemp = 0;
    x = 0;
    y = 0;
    theta = 0;
  ...
  int landmark_id;
  bool is_get_landmark;  // 是否识别到landmark信息
  uint64_t time_stemp;   // 插值得到的时间戳 -- 和该激光雷达的时间戳相同！
  // 相机在二维码坐标系下的位姿
  float x;
  float y;
  float theta;
  Eigen::Matrix<double, 4, 3, Eigen::DontAlign> cov;
  MoveType type;  // 行走模式
  int last_scan_id;  // 和上一帧雷达之间的约束
  int current_scan_id;  // 此帧雷达的信息
};
  ...
  public:
     ...
     * @param y
     */
    Vector2(T x, T y, T instensities)
    {
      m_Values[0] = x;
      m_Values[1] = y;
      m_Values[2] = instensities;
    }
    /**
     * Constructor initializing vector location
     * @param x
  ...
  public:
    ...
    /**
     * Get the instensities of this vector2
     * @param instensities the instensities of the vector2
     */    
    inline const T& GetIntensities() const {
      return m_Values[2];
    }

    inline void SetInstensitiues(const T& instensitiues) {
      m_Values[2] = instensitiues;
    }
  ...
  public:
     ...
     * @return offset pose
     */
    inline const Pose2& GetQrOffsetPose() const
    {
      return m_pQrOffsetPose->GetValue();
    }
     ...
     * @param rPose
     */
    inline void SetQrOffsetPose(const Pose2& rPose)
    {
      m_pQrOffsetPose->SetValue(rPose);
    }
  ...
  private:
    ...
    Parameter<Pose2>* m_pQrOffsetPose;
  };  // Sensor
  ...
  /**
   * Type declaration of intensities readings vector
   */
  typedef std::vector<kt_double> IntensitiesReadingsVector;
  /**
  ...
  public:
    ...
    LaserRangeScan(const Name& rSensorName,
                   ...
                   const IntensitiesReadingsVector& rIntensitiesReads)
      : SensorData(rSensorName)
      , m_pRangeReadings(NULL)
      , m_NumberOfRangeReadings(0)
    {
      ...
      SetIntensitiesReadings(rIntensitiesReads);
    }
  ...
  public:
    ...
    /**
     * Gets the intensities readings of this scan
     * @return intensities readings of this scan
     */
    inline kt_double* GetIntensitiesReadings() const {
      return m_pIntensitiesReadings;
    }
    inline IntensitiesReadingsVector GetIntensitiesReadingsVector() const {
      return IntensitiesReadingsVector(m_pIntensitiesReadings,
            m_pIntensitiesReadings + m_NumberOfRangeReadings);
    }
    ...
    /**
     * @brief: set a intensities from the given sensor with the given readings
     * @return {*}
     */    
    inline void SetIntensitiesReadings(
      const IntensitiesReadingsVector& intensitiesReadings) {
      if (!intensitiesReadings.empty()) {
        assert(intensitiesReadings.size() == m_NumberOfRangeReadings);
        // copy readings
        // if (m_pIntensitiesReadings != nullptr)
        //   delete [] m_pIntensitiesReadings;
        m_pIntensitiesReadings = new kt_double[m_NumberOfRangeReadings];
        kt_int32u index = 0;
        const_forEach(IntensitiesReadingsVector, &intensitiesReadings) {
          m_pIntensitiesReadings[index++] = *iter;
        }
      } else {
        delete [] m_pIntensitiesReadings;
        m_pIntensitiesReadings = NULL;
      }
    }
  ...
  private:
    kt_double* m_pRangeReadings;    // 激光帧原始数据
    kt_double* m_pIntensitiesReadings;  // 光强
    kt_int32u m_NumberOfRangeReadings;
  };  // LaserRangeScan
  ...
  public:
    ...
    LocalizedRangeScan(const Name& rSensorName, const RangeReadingsVector& rReadings,
                      const IntensitiesReadingsVector& rIntensitiesReads)
      : LaserRangeScan(rSensorName, rReadings, rIntensitiesReads)
      , m_IsDirty(true)
    {
    }
  ...
  public:
     ...
     * is usually set by an external module such as a localization or mapping_and_location module when it is determined
    ...
    void SetCovariance(const Matrix3& cov) {
      this->covariance = cov;
    }
    // 增加是否具有二维码为约束
    bool IsGetLandmarkConstraints(LandmarkConstraintsInfo* constraints_info) const {
      *constraints_info = constraints_info_;
      // if (constraints_info_.is_get_landmark) {
      //   std::cout << "get land mark" << std::endl;
      // }
      return constraints_info_.is_get_landmark;
    }
    void SetLandmarkConstraints(const LandmarkConstraintsInfo&
        constraints_info) {
      constraints_info_ = constraints_info;
    }
  private:
    /**
    ...
    virtual void Update()
    {
      ...
      if (pLaserRangeFinder != NULL)
      {
        ...
        for (kt_int32u i = 0; i < pLaserRangeFinder->GetNumberOfRangeReadings(); i++, beamNum++)
        {
          ...
          kt_double intensities = GetIntensitiesReadings()[i];
          if (!math::InRange(rangeReading, pLaserRangeFinder->GetMinimumRange(), rangeThreshold))
          {
            ...
            point.SetInstensitiues(intensities);
  ...
  private:
    ...
    */
    Matrix3 covariance;
    /**
    * 该帧激光数据获取到距离其最近时间的约束信息
    */
    LandmarkConstraintsInfo constraints_info_;
    /**
     * Odometric pose of robot
     */
...
////////////////////////////////////////////////////////////////////////////////////////
  ...
  public:
    ...
    LocalizedRangeScanWithPoints(const Name& rSensorName, const RangeReadingsVector& rReadings,
        ...
        : LocalizedRangeScan(rSensorName, rReadings, rIntensitiesReads),
          m_Points(rPoints)
    {
    }
    ...
    void Update()
    {
      ...
      for (kt_int32u i = 0; i < m_Points.size(); i++)
      {
        ...
        if (!std::isfinite(m_Points[i].GetX()) || !std::isfinite(m_Points[i].GetY()) ||
            !std::isfinite(m_Points[i].GetIntensities())) {
          if (!std::isfinite(m_Points[i].GetIntensities())) {
            std::cout << "aaaaaaaaaaaaaa" << std::endl;
          }
          Vector2<kt_double> point(m_Points[i].GetX(), m_Points[i].GetY(), m_Points[i].GetIntensities());
          m_UnfilteredPointReadings.push_back(point);
        ...
        Vector2<kt_double> point(result.GetX(), result.GetY(), m_Points[i].GetIntensities());
